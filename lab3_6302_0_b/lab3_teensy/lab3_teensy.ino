...
String config_message  = "&A~Desired~5&C&S~K_P~P~0~20~0.05&S~K_D~D~0~10~0.05&S~Direct~O~0~260~0.01&S~Desired~A~-130~130~1&T~ArmAngle~F4~-130~130&T~Error~F4~-150~150&T~Delta~F4~-1000~1000&T~MotorCmd~F4~0~260&H~4&";
...
void loop() {
  ...
  if (loopCounter == numSkip) {  // Lines below are for debugging.
    packStatus(buf, Vangle, error, delta, float(motorCmd), float(headroom));
    Serial.write(buf,22);
    loopCounter = 0;
  } else loopCounter += 1;
